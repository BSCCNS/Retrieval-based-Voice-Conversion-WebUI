...
def train_index(exp_dir1, version19, config_vars = None, 
                                        now_dir = None, 
                                        logger = None):
    # exp_dir = "%s/logs/%s" % (now_dir, exp_dir1)
...
def training_pipeline(p_dict, config_vars = None, 
                                now_dir = None, 
                                logger = None):

    print(f'Runing traning pipeline with parameters: {p_dict}')

    trainset_dir = p_dict['trainset_dir']
    exp_dir = p_dict['trainset_dir']
    sr = p_dict['sr']
    num_proc = p_dict['num_proc']
    ...
    gpus = p_dict['gpus']
    f0method = p_dict['f0method']
    if_f0 = p_dict['if_f0']
    version = p_dict['version']
    gpus_rmvpe = p_dict['gpus_rmvpe']
    ...
    spk_id = p_dict['spk_id']
    save_epoch = p_dict['save_epoch']
    total_epoch = p_dict['total_epoch']
    batch_size = p_dict['batch_size']
    if_save_latest = p_dict['if_save_latest']
    pretrained_G = p_dict['pretrained_G']
    pretrained_D = p_dict['pretrained_D']
    if_cache_gpu = p_dict['if_cache_gpu']
    if_save_every_weights = p_dict['if_save_every_weights']
